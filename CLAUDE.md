...
The app uses a **Producer-Consumer** coordinator architecture with `AppCoordinator` (AppCoordinator.swift:9) as the central orchestrator:

- **AppCoordinator**: Orchestrates the entire workflow using a two-phase approach:
  - **Producer Phase**: High-speed event buffering (keystrokes, shortcuts, screen captures)
  - **Consumer Phase**: AI processing every 3 seconds to avoid performance degradation
- **Flow**: Event → Buffer → (3s interval) → AI Annotation → Nebula Storage → User Decision → Execution → Memory Storage
...
- **Logger** (Logger.swift:33): Centralized logging with categories
  - Categories: Event, Buffer, Annotator, Executor, Nebula, Flow, Capture, System
...
7. Generate stable task ID via SHA256 hash
8. Check if task was previously declined/completed → skip if so
9. **Push comprehensive context to Nebula** with all metadata
10. Show overlays if new task detected

### Task Detection & Overlay Display
1. Task identified as new → `stateStore.updateCurrent(taskId)` returns true
2. **Decision panel** appears (top-right) showing task label
3. **Suggestion panel** shows "Thinking..." near cursor
4. `ExecutorService.generateSuggestionPreview()` calls Grok asynchronously
5. Grok returns specific, actionable suggestion (e.g., "Type and run 'brew restart postgresql'")
6. Suggestion panel updates with AI-generated preview
7. User sees exactly what automation will do before deciding
...
#### 1. Comprehensive Context Flow (Annotator → Nebula → Executor)
